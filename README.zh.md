...
一个 DuckDB 扩展，支持在 SQL 查询中直接读取 Excel、WPS 和 OpenDocument 电子表格文件。该扩展提供了无缝集成，可利用 DuckDB 强大的 SQL 引擎分析电子表格数据。

[English README is here](https://github.com/redraiment/rusty-sheet/blob/main/README.md)

## 功能特性

- **高性能**：采用纯 Rust 实现，针对大文件优化；例如，在 MacBook M1 上读取 100 万行的 XLSX 文件现在只需不到 13 秒（相比 v0.1.x 版本的 140 多秒有显著提升）。
- **多格式支持**：读取 Excel 文件（`.xls`, `.xlsx`, `.xlsm`, `.xlsb`, `.xla`, `.xlam`）、WPS 文件（`.et`, `.ett`）以及 OpenDocument 电子表格文件（`.ods`）
- **批处理**：使用通配符模式匹配分析和读取多个文件和工作表
- **灵活的数据类型**：支持布尔值、整数、双精度浮点数、可变长度字符串、日期时间、日期和时间类型
- **Excel 风格的数据范围**：使用熟悉的 Excel 表示法指定数据范围（例如 `"A1:C3"`）
- **自动列类型检测**：自动推断列类型；可使用 `columns` 参数覆盖特定列的类型
- **标题行处理**：自动检测和解析标题行
- **错误处理**：可配置的解析错误行为，并提供精确的单元格位置报告
- **类型安全**：内置数据类型验证和转换
- **纯 Rust 实现**：无外部依赖，充分利用 Rust 的内存安全性和性能优势
- **高级数据过滤**：跳过空行或在遇到第一个空行时停止读取，以实现高效数据处理
...
- **sheet_name**（可选，默认为第一个工作表）：工作表名称（支持通配符，如 `Sheet*`）
...
- **file_pattern**（必需）：支持通配符的文件路径模式（例如 `*.xlsx`, `*.et`）
...
| `timestamp` | TIMESTAMP | 具有微秒精度的日期和时间（支持 ISO 8601 格式） |
| `date` | DATE | 不包含时间成分的日期（支持 ISO 8601 格式） |
| `time` | TIME | 不包含日期成分的时间（包括 ISO 8601 持续时间） |
...
1. **起始列**（可选）：Excel 列字母（例如 `A` 表示第 1 列，`B` 表示第 2 列，依此类推）
    - 如果指定，即使该列不包含数据，也会从此列开始读取
    - 不会跳过空列

2. **起始行**（可选）：Excel 行号（例如 `1` 表示第 1 行，`2` 表示第 2 行，依此类推）
    - 如果指定，即使该行不包含数据，也会从此行开始读取
    - 不会跳过空行

3. **冒号分隔符**：仅在指定结束列或结束行时需要

4. **结束列**（可选）：Excel 列字母
    - 如果指定，即使数据在此之前结束，也会在此列处停止读取
    - 忽略超出此列的数据

5. **结束行**（可选）：Excel 行号
    - 如果指定，即使数据在此之前结束，也会在此行处停止读取
    - 忽略超出此行的数据
...
该扩展支持使用 Rust glob 模式进行文件和工作表匹配。
